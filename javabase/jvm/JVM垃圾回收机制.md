...
**思考：对象不可达GC Roots就一定会被回收吗？**

答案：当然不是，这时候它们暂时处于“缓刑”阶段，要真正宣告一个对象死亡，至少要经历两次标记过程：如果对象在进行可达性分析后发现没有与GC Roots相连接的引用链，那它将会被第一次标记并且进行筛选，筛选的条件是次对象是否有必要执行finalize()方法。当对象没有覆盖finalize()方法，或者finalize()方法没有被虚拟机调用过，虚拟机将这两种情况都视为“没有必要执行”。
